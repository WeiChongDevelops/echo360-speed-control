(function() {
  ...
  function etaPollSnapshot(): void {
    if (etaPollInFlight) return;
    if (!ETA_VISIBLE_STATES.includes(currentStatusState)) return;
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.tabs.sendMessage(tabs[0].id!, { action: 'getSnapshot' }, (response: SpeedResponse) => {
        etaPollInFlight = false;
