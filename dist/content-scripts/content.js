...
(function () {
    ...
    chrome.storage.onChanged.addListener((changes, areaName) => {
        if (areaName !== 'local' || !('totalTimeSavedSeconds' in changes))
            return;
    ...
    window.addEventListener('message', (event) => {
        ...
        if (event.data.type === 'SPEED_CHANGED') {
            ...
            if (!Number.isFinite(delta))
                ...
                reason = 'delta not positive';
            else if (!Number.isFinite(claimed))
                reason = 'claimed wall not finite';
            else if (claimed <= 0)
                reason = 'claimed wall not positive';
            ...
            else if (observedWall < 1)
                reason = 'batch arrived under the 1 s accept interval';
            else if (delta > bound)
                reason = 'delta exceeds the wall-time bound';
            if (reason !== null) {
