...
(function () {
    ...
    function showSpeedOverlay(speed, phrase, retryCount = 0) {
        if (!speedOverlay) {
        ...
        if (video && classifyDuration(video.duration) === 'finite' && !video.ended) {
            ...
            if (remaining > 0)
                text = `${head}, ${etaMainPart(remaining)}`;
        }
    ...
    function noteAppliedSpeed(s) {
        ...
        if (s !== 1 && s >= 0.25 && s <= 4)
            previousStudySpeed = s;
    }
    function toggleStudySpeed() {
        const video = document.querySelector('video');
        if (!video)
            return;
        ...
        if (current !== 1) {
            ...
            window.setSpeed(previousStudySpeed);
    ...
    `;
        ...
        const makeRow = (emoji, secondKey, trailingLabel) => {
